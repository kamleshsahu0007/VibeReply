...
1. **`/api/subscription-status` aur `/api/stripe/create-checkout-session` par rate-limiting nahi
   hai.** In dono par abhi bhi `assertRateLimit()` call nahi hoti (sirf `generate-replies` aur
   `tones*` par hai). In dono ki cost kam hai (subscription-status ek DB read hai, create-checkout-session
   Stripe API call hai jo Stripe khud rate-limit karta hai), lekin spam/DoS-style abuse se
...
10. Root-level `README.md` (repo ke root me, is `docs/` folder se alag) **kuch purana ho chuka
    ...
    datasource actually `postgresql` hai. Ye README `STRIPE_*` env vars aur `/api/stripe/*`,
...
11. **Stripe global subscriptions indefinite recurring hain** — Stripe native recurring subscriptions
    handle karta hai, jisme fixed cycles ka restriction nahi hota. Automatic recurring billing
    chalti hai jab tak user cancel na kare.
12. **Koi login/account system nahi hai** — identity purely device-scoped UUID hai. Agar user
    ...
    subscription Stripe customer portal ya ID se dobara link na ho — koi account-recovery flow nahi hai).

## Suggested next steps (priority order, agar koi is par kaam kare)

1. `subscription-status` aur `stripe/create-checkout-session` par bhi rate-limiting add karo
